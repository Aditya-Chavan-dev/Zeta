# Autonomous Engineering Governance Rule (Always Active)
...
2. **If `.zeta/state.json` exists**:
   ...
   - You MUST read `.zeta/state.json` using `view_file` or check its active step before taking any actions.
...
3. **If `.zeta/state.json` does NOT exist**:
   - If the user asks to start a new greenfield project or design software, prompt to initialize the governance lifecycle with Step 0 (Project Intent).

## Strict Invariants
- Never allow manual code edits that violate the active step's stage gate.
- Always require the explicit human handshake (`"Approve"`) to lock steps and advance.
- Always present Top 3 industry options for architectural trade-offs.
