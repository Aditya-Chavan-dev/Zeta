...
describe('End-to-End 6-Stage Lifecycle: Agent 01 -> Agent 02 -> Agent 03 -> Agent 04 -> Agent 05 -> Agent 06', () => {
  const testDir = path.join(process.cwd(), '.test-e2e-6stages');
  ...
  test('Executes full 6-agent chain with strict gating and zero state loss', async () => {
    StateManager.initialize(testDir, 'e2e-6stages');
    ...
    assert.ok(fs.existsSync(path.join(testDir, 'docs', 'SYSTEM_ARCHITECTURE_BLUEPRINT.md')));
    ...
    assert.ok(fs.existsSync(path.join(testDir, 'docs', 'DETAILED_TECHNICAL_DESIGN.md')));

    // === Final State Verification ===
    const state = StateManager.load(testDir);
    assert.ok(state);
    assert.equal(state.activeStep, 6);
    assert.deepEqual(state.lockedSteps, [0, 1, 2, 3, 4, 5]);
    ...
    assert.ok(state.stepSummaries['step_4']);
    assert.ok(state.stepSummaries['step_5']);
  });
});
