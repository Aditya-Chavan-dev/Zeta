...
describe('End-to-End 7-Stage Lifecycle: Agent 01 -> ... -> Agent 07', () => {
  const testDir = path.join(process.cwd(), '.test-e2e-7stages');
  ...
  test('Executes full 7-agent chain with strict gating and zero state loss', async () => {
    StateManager.initialize(testDir, 'e2e-7stages');
    ...
    assert.ok(fs.existsSync(path.join(testDir, 'docs', 'IMPLEMENTATION_PLAN_AND_WBS.md')));

    // === Final State Verification ===
    const state = StateManager.load(testDir);
    assert.ok(state);
    assert.equal(state.activeStep, 7);
    assert.deepEqual(state.lockedSteps, [0, 1, 2, 3, 4, 5, 6]);
    for (let i = 0; i <= 6; i++) {
