...
describe('End-to-End 5-Stage Lifecycle: Agent 01 -> Agent 02 -> Agent 03 -> Agent 04 -> Agent 05', () => {
  const testDir = path.join(process.cwd(), '.test-e2e-5stages');
  ...
  test('Executes full 5-agent chain with strict gating and zero state loss', async () => {
    StateManager.initialize(testDir, 'e2e-5stages');
    ...
    while (!a5Resp.isReadyForSignoff) {
      a5Resp = await agent5.handleTurn('1');
    }
    ...
    assert.deepEqual(state.lockedSteps, [0, 1, 2, 3, 4]);
