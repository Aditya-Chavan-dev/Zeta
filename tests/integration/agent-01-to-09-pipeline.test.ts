...
describe('End-to-End 9-Stage Lifecycle: Agent 01 -> ... -> Agent 09', () => {
  const testDir = path.join(process.cwd(), '.test-e2e-9stages');
  ...
  test('Executes full 9-agent chain with strict gating and zero state loss', async () => {
    StateManager.initialize(testDir, 'e2e-9stages');
    ...
    assert.deepEqual(state.lockedSteps, [0, 1, 2, 3, 4, 5, 6, 7, 8]);
    for (let i = 0; i <= 8; i++) {
