...
export class PreconditionVerifier {
  /**
   * Verifies that Steps 0 through 7 are all locked in session state.
   */
  public static verifyPrerequisites(workspaceRoot: string): PreconditionVerificationResult {
    ...
    const s6 = state.stepSummaries['step_6'];
    const s7 = state.stepSummaries['step_7'];

    if (!s0?.summary || !s1?.summary || !s2?.summary || !s3?.summary || !s4?.summary || !s5?.summary || !s6?.summary || !s7?.summary) {
      return {
        isValid: false,
        step0Tldr: '',
        step1Tldr: '',
    ...
    return {
      ...
      step6Tldr: s6.summary,
      step7Tldr: s7.summary
    };
  }
}
